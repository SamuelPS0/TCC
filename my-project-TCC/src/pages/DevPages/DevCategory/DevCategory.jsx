...
const DevCategory = () => {
  ...
  const carregarCategorias = async () => {
    ...
    try {
      ...
      toast.error("Erro ao carregar categorias.", { id: toastId });
  ...
  useEffect(() => {
    carregarCategorias();
    // Mantém o loading até o fade-out terminar
  ...
  const criarCategoria = async (postdata) => {
    try {
      ...
      toast.error("Houve um erro ao registrar a categoria, tente novamente.");
    }
  };
  ...
  if (carregando) return <Loading onComplete={() => setCarregando(false)} />;
